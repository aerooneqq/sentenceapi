...
namespace SentenceAPI.ApplicationFeatures.Loggers.Configuration
{
    ...
    {
        public const string LogLevelPrefix = "LL";
        public const string DelimiterPrefix = "Del";
        public const string ExtraLinePrefix = "ExtraLine";
        public const string LogInformationPrefix = "LogInformation";

        private static string[] prefixes = { LogLevelPrefix, DelimiterPrefix, ExtraLinePrefix, LogInformationPrefix };

        private static Dictionary<string, LogLevel> stringToLogLevelDict = 
        ...
        private static Dictionary<string, LogInformation> stringToLogInformationnewDict = 
        ...
        private void ReadLogConfigurationFile(string[] lines) 
        {
            for (int i = 0; i < lines.Length;) 
            {
                ...
                while (i < lines.Length && lines[i].Length != 0) 
                {
                    ...
                    {
                        case LogLevelPrefix:
                            string logLevelType = line.Substring(line.IndexOf(LogLevelPrefix) + LogLevelPrefix.Length + 1);
                        ...
                        case DelimiterPrefix:
                            ...
                            string delimiter = line.Substring(line.IndexOf(DelimiterPrefix) + DelimiterPrefix.Length + 1);
                        ...
                        case LogInformationPrefix:
                            ...
                            string logDataString = line.Substring(line.IndexOf(LogInformationPrefix) 
        ...
        private LogInformation GetLogInformation(string logInformationString)
        {
            try 
            {
                return stringToLogInformationnewDict[logInformationString];
            }
            catch
            {
                ThrowInvalidDataException();
